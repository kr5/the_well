...
**Extended and partially corrected by:** [`PRD-V3-COMPREHENSIVE-EXPANSION.md`](PRD-V3-COMPREHENSIVE-EXPANSION.md). Read that addendum alongside this document — it fixes an important scoping gap here (this document implicitly assumes the Election Commission of India is the only relevant authority; Panchayat/Municipal elections are actually run by separate State Election Commissions, see its Section V3) and adds optional accounts/drafts/privacy, existing-voter parity, a self-improving content pipeline, and new open-source research (Bhashini, DIGIT, India Stack).
