...
| `migrations/0001`-`0013` | Full Postgres schema: KB + audit trail, decision trees, forms/geography/jurisdiction, elections calendar + MCC windows, admin/RBAC/audit log, accounts/drafts/consent, feedback/fringe cases, analytics, link-check/translation status, bot channel config, account OTP/sessions, KB translation grouping + FAQ tagging | Written, not run against a live database |
| `crates/web-app` (`voteassist-web-app` binary, built via `cargo-leptos`) | Public site: Leptos SSR + islands, full sitemap from `docs/05-information-architecture.md` (home, `/start`, `/search`, `/learn` + forms/faq/glossary/`:slug`, `/locate`, four `/about/*` pages, `/feedback` writing to Postgres, `/accessibility`, `/account` + `/account/login`). Server functions call `core-domain`/`kb-content` in-process — no separate HTTP hop to `crates/api`. Includes the optional public account system (email OTP via `lettre`/SMTP, HMAC-blind-indexed contact hash, argon2 OTP hashing), saved/frozen decision-tree "checklists," and a cookie-consent banner that gates real `analytics::record_event` calls (session start/question answered/terminal reached/KB search/KB entry viewed — deep-link-click tracking is a disclosed remaining gap) via a plain, non-`HttpOnly` mirror cookie server-side code can read | Written, not locally compiled — see `src/server_fns.rs`/`src/render.rs`/`src/accounts/` module docs for the client-held-session architecture, the disclosed reason `channel-core` isn't reused here, and the DPDP/cookie-consent design |
| `crates/admin-app` (`voteassist-admin-app` binary, built via `cargo-leptos`) | Admin dashboard, a separate deploy target from `web-app`: hand-rolled Postgres-backed session auth (argon2 + a CSPRNG token — not `tower-sessions`, see `src/auth/session.rs` for why), RBAC (`require_role`, enforced per server function, not just hidden UI). All 13 PRD v2 Section 11 admin pages: Dashboard, Knowledge Base Content Editor, Decision Tree Visual Editor (JSON-based, not a node-graph canvas — disclosed simplification), Translation Management, Citation & Link Health, Feedback & Grievance Triage, MCC Control Panel, Bot Channel Management, User & Role Management, Audit Log Viewer, Analytics Dashboard, Data Export & Retention Tools | Written, not locally compiled — see this crate's README for exactly what each page does and its disclosed scope boundaries |
...
a real, if not-yet-compiled, implementation, and `crates/admin-app` now
covers all 13 admin pages from PRD v2 Section 11 — see
`crates/admin-app/README.md`'s "Disclosed scope boundaries within pages"
section for the handful of narrower, deliberate simplifications within
individual pages (the Decision Tree Visual Editor is a JSON editor rather
than a node-graph canvas; Translation Management has no in-browser
per-string editor; a few pages are missing one cross-linking convenience
each). None of these are missing backends — every page reads/writes data
this workspace already produces (`jobs`, `analytics`, the migrations).
See PRD v2 Section 21 and PRD v3 Section V8 (EPICs 17-22) for the original
sequencing this now supersedes.
